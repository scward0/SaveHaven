...
/**
 * User preferences screen with Material Design switches
 * Saves settings to Firestore for sync across devices
 */
class PreferencesActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {

    // All our preference switches
    ...
    // Prevent listener callbacks during initial setup
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Load saved preferences from Firestore first
        db.collection("userPreferences").document(uid).get()
            .addOnSuccessListener { document ->
                if (document != null && document.exists()) {
                    // Set switches to saved values (or defaults)
                    ...
                    // Also update local preferences for education tips
                ...
                // NOW we can activate the listeners (after setting initial states)
    ...
    // Link all the switch components
    ...
    // Set up listeners for each switch to save changes
    private fun setupSwitchListeners(db: FirebaseFirestore, uid: String) {
        ...
        // Educational tips (also update local preferences)
        switchFunFacts.setOnCheckedChangeListener { _, isChecked ->
            if (listenersActive) {
                saveToFirestore(db, uid, "education_facts", isChecked)
                // Also save locally so MainActivity can read it immediately
    ...
    // Show user-friendly confirmation messages
